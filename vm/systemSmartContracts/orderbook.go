...
func (ob *OrderBook) processLimitOrder(limitOrder *Order) (done *Done, err error) {
	...
	order := ob.GetOrder(limitOrder.GetID())
	if order != nil {
	...
	if limitOrder.GetTIF() == FOK {
		if !side.CanOrderBeFilled(limitOrder.GetSide(), limitOrder.Price, quantity) {
			limitOrder.Cancel()
	...
	if limitOrder.GetTIF() == IOC && quantity.GreaterThan(fpdecimal.Zero) {
		limitOrder.SetTaker()
		done.appendCanceled(ob.CancelOrder(limitOrder.GetID()))
		done.Stored = false
	}
...
func (ob *OrderBook) processStopOrder(stopOrder *Order) (done *Done, err error) {
	...
	ob.orders[stopOrder.GetID()] = stopOrder
...
func (ob *OrderBook) checkOCO(order *Order, done *Done) bool {
	...
	_, ok := ob.OCO[order.GetID()]
	if !ok {
		return false
	}

	delete(ob.OCO, order.GetID())

	ob.cancelOCO(order.GetID(), done)
...
func (ob *OrderBook) appendToOCO(order *Order, done *Done) {
	...
	ob.OCO[order.GetOCO()] = struct{}{}

	ob.cancelOCO(order.GetOCO(), done)
}
